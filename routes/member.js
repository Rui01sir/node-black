...
router.put("/register", async (req, res) => {
	...
	const sql =
		"INSERT INTO `membership`( `mem_Name`, `mem_Email`, `mem_Password`, `mem_Birthday`, `mem_Mobile`, `mem_City`, `mem_Area`, `mem_Address`) VALUES (?,?,?,?,?,?,?,?,1,NOW(),null)";
	// let memHeadshot = "";
	// if (req.body.mem_gender === "男") {
	// 	memHeadshot = "male.jpg";
	// } else if (req.body.mem_gender === "女") {
...
router.post("/login", async (req, res) => {
	...
	const sql = "SELECT * FROM membership WHERE mem_Email=?";
	...
	// 比對密碼
	if (!(req.body.password === result[0].mem_Password)) {
		output.code = 402;
		output.error = "密碼錯誤";
		return res.json(output);
	} else {
		...
		output.code = 200;
		output.error = "";
		output.member_Token = jwt.sign(
			{
				member_sid: result[0].member_sid,
				mem_Account: result[0].mem_Email,
			},
			process.env.JWT_SECRET
		);
		output.member_sid = result[0].member_sid;
		output.mem_Account = result[0].mem_Email;
	}
	console.log(output);
	res.json(output);
});
